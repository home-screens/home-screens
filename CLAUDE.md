...
- Stores participating in family changes or backup restore share `src/lib/data-transaction.ts`, a reentrant coordinator with a cross-process file lock. Family migration, removal and backup restore use a durable journal with recoverable before/after images. Keep reference checks and their writes inside the coordinator; new multi-file write paths must participate too. Server startup pins the data root, and the lock is a sibling of the release directory (`<app-dir>.data.lock`) so a deploy swap cannot carry it along. The lock itself is `src/lib/data-lock.ts`: one writer wins a hard link, the file names its owner, and a writer proves it still holds the lock by finding its own token there before every write and before releasing. There is no heartbeat and nothing expires on elapsed time (a Pi's clock jumps hours when the network comes up); an owner is finished with only when its process is gone, so a paused writer keeps its lock, a killed one is replaced at once, and a writer releasing its own lock cannot have had it taken meanwhile. `scripts/upgrade.sh` implements the same file format so a deploy and the server exclude each other. Runtime-only timer sessions, telemetry and backup reminders use per-file atomic writes without the global lock or fsync.
