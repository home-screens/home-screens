...
nextjs:
  metadata:
    title: Plugin development
    description: Build plugins for Home Screens. Runtime-loaded modules with an SDK, a server-side API proxy, secrets, auth adapters, shared state, and a config UI.
    alternates:
      canonical: /docs/plugin-development
---

This page is for people writing a plugin. To install one, see the [Plugins guide](/docs/plugins).
