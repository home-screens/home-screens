...
| `textScale` | number |, | **Text size**, a percent (50–450) of what the module shows on its own: the fitted size on the modules that fit their text to their box, the registry base `fontSize` everywhere else. Unset = the stored `fontSize` stands. Setting it in the editor resets `fontSize` to the base |
