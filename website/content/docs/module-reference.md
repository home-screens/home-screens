...
| `fontSize` | number | `16` | Base text size in pixels. Not edited directly any more: the editor's **Text size** slider is a percent of the module's base (see `textScale`). A module that has only this value keeps its old meaning, the size on most modules and a floor on the ones that fit their text to their box |
| `textScale` | number |, | **Text size**, a percent of the module's normal size (50–450). On modules that fit their text to their box (clock, countdown, date, greeting, weather, news, quote, dad joke, word of the day, this day in history, affirmations, sticky note, to-do list, multi-month calendar) 100 is the fitted size; everywhere else it is the base `fontSize` from the registry. Unset = 100. Setting it in the editor resets `fontSize` to the module's base |
