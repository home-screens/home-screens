...
- **Text size**: the module's text size as a percent of its base size, from 50% to 450%. A module that was given a pixel size in an older version shows that size as a percent (31px on a 16px base reads 194%), so nothing changes until you move the slider. The modules that fit their text to their box (the clock, date, countdown, greeting, weather, news, quote, dad joke, word of the day, this day in history, affirmations, sticky note, to-do list and multi-month calendar) treat it as the smallest their text can be and grow past it to fill the box, so on a large card the text only changes once the slider is above the size that already fits
