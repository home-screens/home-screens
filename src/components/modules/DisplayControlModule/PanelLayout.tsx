  ...
  return (
    <div className="h-full w-full flex flex-col p-5 gap-3">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        <LabeledButton aria={t('display-control.ariaPrev')} label={t('display-control.prev')} onClick={onPrev}>
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M15 19l-7-7 7-7" /></svg>
        </LabeledButton>
        <LabeledButton aria={t('display-control.ariaNext')} label={t('display-control.next')} onClick={onNext}>
        ...
        >
          ...
          <span className="text-sm">{t('display-control.sleep')}</span>
        </HoldConfirmButton>
      </div>

      <div className="space-y-1.5 mt-auto">
        <div className="flex items-center justify-between">
          <span className="text-xs text-hs-text-muted uppercase tracking-wider">{t('display-control.brightness')}</span>
