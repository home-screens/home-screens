  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ width: s * 21, height: s * 21, display: 'grid', placeItems: 'center', marginBottom: u * 1.4, position: 'relative' }}>
        ...
        <div style={{ fontSize: s * 5.2, fontWeight: 400, letterSpacing: '-.02em', marginTop: u * 2.5 }}>{now?.description ?? ''}</div>
      ...
      {chips.length > 0 && (
        <div style={{ flex: 'none', display: 'grid', gridTemplateColumns: `repeat(${chips.length}, 1fr)`, gap: u * 1.3 }}>
          {chips.map((d) => {
            const Ico = getWeatherIcon(d.icon, 'outline');
            return (
              <div key={d.date} style={{
                textAlign: 'center', padding: `${u * 1.7}px ${u * .6}px`, borderRadius: u * 1.7,
